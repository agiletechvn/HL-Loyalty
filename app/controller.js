...
module.exports = (function() {
	return {
		...
		query: function(req, res) {
			...
			// setup the fabric network
			var channel = fabric_client.newChannel(req.query.channel);
			var peer = fabric_client.newPeer("grpc://localhost:7051");
			channel.addPeer(peer);
			...
			Fabric_Client.newDefaultKeyValueStore({
				...
				.then(state_store => {
					...
					// get the enrolled user from persistence, this user will sign all requests
					return fabric_client.getUserContext(req.query.user, true);
				})
				.then(user_from_store => {
					if (user_from_store && user_from_store.isEnrolled()) {
						console.log(
							"Successfully loaded " + req.query.user + " from persistence"
						);
						member_user = user_from_store;
					} else {
						throw new Error(
							"Failed to get " + req.query.user + ".... run registerUser.js"
						);
					}
				...
				.then(query_responses => {
					...
					if (query_responses && query_responses.length == 1) {
						if (query_responses[0] instanceof Error) {
							...
							res.json({ result: query_responses[0].toString() });
						}
					} else {
						console.log("No payloads were returned from query");
						res.send("Could not run query");
